...
function addProductItem(product) {
  const folders = product.productFolderList.map(folder =>
    `
            <span onclick="openFolder(${folder.folderId})">
                #${folder.name}
            </span>
        `
  );
  return `<div class="product-card">
                ...
                <div class="product-tags" style="margin-bottom: 20px;">
                    ${folders}
                    <span onclick="addInputForProductToFolder(${product.productId}, this)">
...
function addInputForProductToFolder(productId, button) {
  $.ajax({
    ...
    success: function (folders) {
      const options = folders.map(folder => `<option value="${folder.folderId}">${folder.name}</option>`)
      const form = `
                <span>
